  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" class="user-table">
      ...
      <el-table-column align="center" label="性别">
        <template slot-scope="scope">
          <span>{{ scope.row.gender | genderStatus }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="角色">
        <template slot-scope="scope">
          <el-tag :type="scope.row.roles | roleFilter" size="small">{{ scope.row.roles | userRoleStatus }}</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" label="状态">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status | statusFilter" size="small">{{ scope.row.status | userStatus }}</el-tag>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    handleDetail(row) {
      this.detailList = [
        ...
        { key: '性别', value: row.gender === 'male' ? '男' : '女' },
        ...
        { key: '注册时间', value: row.regTime ? parseTime(row.regTime, '{y}-{m}-{d} {h}:{i}') : '' },
        { key: '最近登录时间', value: row.loginTime ? parseTime(row.loginTime, '{y}-{m}-{d} {h}:{i}') : '' },
